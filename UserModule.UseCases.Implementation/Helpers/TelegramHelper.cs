...
namespace UserModule.UseCases.Implementation.Helpers;

internal sealed class TelegramHelper(IConfiguration configuration) : ITelegramHelper
{
    private readonly string _botToken = configuration.GetSection("TelegramBotApiKey").Value ?? throw new InvalidOperationException();
